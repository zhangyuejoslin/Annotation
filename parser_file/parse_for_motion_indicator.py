...
def parsing(json_file):
    ...
    for each_sentence in json_file['_views']['_InitialView']['Sentence']:
        ...
        if sentence[each_sentence['end']] == '\n':
            sentence_list.append((find_sentence[0], each_sentence['end']))
            find_sentence = []
    ...
    for each_sentence_list in sentence_list:
        sentence_text.append(sentence[each_sentence_list[0]:each_sentence_list[1]])
    new_sentence_list = zip(sentence_list,sentence_text)
    ...
    for each_indicator in indicator_list:
        ...
        for rel in relations:
            ...
            if str(rel['Dependent']) in trajector_and_entity and str(rel['Governor']) in trajector_and_entity:
                ...
                if 'Motions' in rel.keys():
                    Configure['config' + str(item_index)]['Type'] = "Motion:" + rel['Motions']
        item_index+=1
    return Configure
...
if __name__ == '__main__':
        ...
        for sentence in parsing(load_dict):
            print(sentence)
            # form_representation_generation(sentence)
            # import sys
            # sys.exit()
